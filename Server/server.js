...
const sendRecoveryEmail = (firstname, lastname, email, token, req, res) => {
    ...
    let style =
        "background-color: green; color: white; width: 4em; text-decoration: none; padding: 1vh 1vw; border-radius: 5%;";
    let mailOptions = {
        to: email,
    ...
    wrapper.isUniqueUsername(username).then(unique => {
        if (!unique) {
            ...
            if (phonenumber != null && email != null) {
                ...
                wrapper
                    .postUserObject(firstname, lastname, username, password)
                    .then(result => {
                        tokenReturn(res, result);
                        return;
                    });
...
app.put("/change/user/", (req, res) => {
    ...
    for (let i in keys) {
        ...
        if (value != null) {
            wrapper
                .changeTable("Users", key, value, user_id, "user_id")
                .then(result => {
                    if (!result) {
                        return res.status(500).json({
                            message: "insertion failure"
